...
import { PontoLuz } from './light/PontoLuz.js';
...
const pointLightHelper = new THREE.PointLightHelper(PontoLuz);
...
const gui = new GUI({
  ...
  closeFolders: true  
    ...
    const lightFolder = gui.addFolder('SpotLight');
		lightFolder.addColor( new ColorGUIHelper( PontoLuz, 'color' ), 'value' ).name( 'color' );
		lightFolder.add( PontoLuz, 'intensity', -1, 100 );
    lightFolder.add(PontoLuz.position,'x',-5,5);
    lightFolder.add(PontoLuz.position,'y',-5,5);
    lightFolder.add(PontoLuz.position,'z',-5,5);
